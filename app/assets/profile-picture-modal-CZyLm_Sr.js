import{g as he,e as h,d as le,h as Z,j as ne,k as se,a as xe,s as R,l as _e,m as ke,n as we,o as pe,A as Le,q as ae,r as Se,v as Ae}from"./main-M219EGKV.js";function Ee(e){const t=!!e?.profile_pictures_ai_enabled;return{masterOn:t,usable:t&&e?.ai_provider_openai!==!1}}async function ze(e,t={}){const{onSaved:b,showCustomAlert:k}=t,c=window.__flangoGetInstitutionById?.(e.institution_id);if(!c)return;let g=c.profile_picture_types||["upload","camera","library"];const o=e.id!=="__default__"&&await he(c),n=document.createElement("div");n.className="profile-pic-modal-overlay";const r=document.createElement("div");r.className="profile-pic-modal";const y=e.number?` (${e.number})`:"";r.innerHTML=`
    ...
    `,t.querySelector(".profile-pic-flango-back")?.addEventListener("click",o),ke({root:t.querySelector(".profile-pic-flango-root"),userId:b.id,api:we,managedUserName:b.name,allowRelease:!0,onAvatarChanged:async(r,y={})=>{if(!["claimed","released"].includes(y.reason))return;const l=r?.assignment?{profile_picture_url:r.assignment.image_path,profile_picture_type:"library",profile_picture_updated_at:new Date().toISOString()}:{profile_picture_url:null,profile_picture_type:null,profile_picture_updated_at:new Date().toISOString()};Object.assign(b,l),pe(b.id),await ce(c,b),g?.(l)}}).open()}async function ce(e,t){if(!(t.profile_picture_url&&!t.profile_picture_opt_out)){e.innerHTML=`
        ...
        <div class="profile-pic-subview">
            ...
            <div id="pp-camera-status" class="profile-pic-loading" style="text-align:center;margin-top:8px;">
                ...
                </div>`,r.querySelector("#pp-camera-save").addEventListener("click",async()=>{r.innerHTML='<div class="profile-pic-loading"><span class="profile-pic-spinner"></span> Uploader...</div>';const w=await ne(A,t.institution_id,t.id,"camera",t.name);URL.revokeObjectURL(H),w.success?(t.profile_picture_url=w.storagePath||`${t.institution_id}/${t.id}.webp`,t.profile_picture_type="camera",c&&c({profile_picture_url:t.profile_picture_url,profile_picture_type:"camera"}),k()):r.innerHTML=`<div style="color:#f87171;text-align:center;">${h(w.error||"Upload fejlede")}</div>`}),r.querySelector("#pp-camera-retry").addEventListener("click",()=>{URL.revokeObjectURL(H),r.style.display="none",l.style.display="block",n.parentElement.style.display="flex"})},"image/jpeg",.9)})}function Ue(e,t,b,k){let c=null;const g=Le.map((r,y)=>`
    ...
    `).join("");e.innerHTML=`
        ...
        </div>`;const c=await xe(t.institution_id);if(!c||c.length===0){e.innerHTML=`
            <div class="profile-pic-subview">
    ...
    `).join("");e.innerHTML=`
        ...
        <div class="profile-pic-subview">
            ...
            </div>`}).join("");v?w.innerHTML=v:w.innerHTML='<div style="color:#94a3b8;font-size:11px;padding:8px;">Ingen eksisterende billeder. Brug kamera eller upload ovenfor.</div>',w.querySelectorAll("[data-lib-index]").forEach(a=>{a.addEventListener("click",async()=>{const S=parseInt(a.dataset.libIndex),_=i[S];if(_){a.querySelector("img").style.borderColor="#f59e0b";try{const L=p.get(_.id)||_.storage_path,j=await(await fetch(L)).blob();w.style.display="none",V(j,"Eksisterende billede sendes til AI og slettes straks efter","library")}catch{a.querySelector("img").style.borderColor="#f87171",setTimeout(()=>{a.querySelector("img").style.borderColor="transparent"},1500)}}})})})}export{ze as openProfilePictureModal};
