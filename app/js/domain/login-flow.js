...
import { rememberInstitution, ensureActiveInstitution, fetchInstitutions } from './institution-store.js?v=3.0.69';
import { getInstitutionId } from './session-store.js?v=3.0.69';
import { performLogin, getCurrentUserProfile } from './auth-and-session.js?v=3.0.69';
...
export async function setupFullLoginScreen({ fromDeviceUnlock = false } = {}) {
    ...
    loginBtn.onclick = async () => {
        ...
        if (alreadyRegistered) {
            const { setupAdminLoginScreen } = await import('./app-main.js?v=3.0.69');
...
export function setupPinLockedScreen(adminName) {
    ...
    loginBtn.onclick = async () => {
        ...
        const alreadyRegistered = existingDeviceUsers.some(u => u.userId === session.user.id);
        if (alreadyRegistered) {
            const { setupAdminLoginScreen: startAdmin } = await import('./app-main.js?v=3.0.69');
