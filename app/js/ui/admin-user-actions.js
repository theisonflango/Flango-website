export function createAdminUserActions(options = {}) {
    ...
    async function handleDeposit(userId) {
        ...
        if (amount === null) return; // Brugeren annullerede
        ...
        showCustomAlert('Success!', `Indbetaling på ${amount.toFixed(2)} kr. til ${user.name} er gennemført.`);
    }
    ...
    async function handleEditBalance(userId) {
        ...
        if (newBalance === null) return; // Brugeren annullerede
        ...
        updateSelectedUserInfo(); // Opdater hvis brugeren er valgt
        playSound('balanceUpdate');
        showCustomAlert('Success!', `${user.name}'s saldo er blevet sat til ${newBalance.toFixed(2)} kr.`);
    }
