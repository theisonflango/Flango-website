...
import { getCurrentClerk, getCurrentAdmin, isCurrentUserAdmin, getInstitutionId } from '../domain/session-store.js?v=3.0.79';
import { getProductIconInfo, applyProductLimitsToButtons, invalidateChildLimitSnapshot } from '../domain/products-and-cart.js?v=3.0.79';
import { setupHelpModule, openHelpManually } from './help.js?v=3.0.79';
import { supabaseClient, SUPABASE_URL, SUPABASE_ANON_KEY } from '../core/config-and-supabase.js?v=3.0.79';
import { getCurrentCustomer } from '../domain/cafe-session-store.js?v=3.0.79';
import { getOrder } from '../domain/order-store.js?v=3.0.79';
import { getMyDeviceTokens, revokeDeviceToken, revokeAllDeviceTokens, clearAllDeviceUsers } from '../domain/device-trust.js?v=3.0.79';
...
import {
    ...
    isThemePackTheme,
    ALL_VALID_THEMES
} from './theme-loader.js?v=3.0.79';
import { initMobilePayImport, injectStyles as injectMobilePayStyles } from '../domain/mobilepay-import.js?v=3.0.79';
import { updateInstitutionCache } from '../domain/institution-store.js?v=3.0.79';
import { showCustomAlert } from './sound-and-alerts.js?v=3.0.79';
import { refetchAllProducts } from '../core/data-refetch.js?v=3.0.79';
import { invalidateAllLimitCaches } from '../domain/purchase-limits.js?v=3.0.79';
import { getCafeEventSettings, saveCafeEventSettings } from '../domain/cafe-events.js?v=3.0.79';
import { openAulaImportModal } from './aula-import-modal.js?v=3.0.79';
import { openUserAdminPanel, openParentPortalAsAdmin } from './user-admin-panel.js?v=3.0.79';
...
function renderProductRulesTable() {
    ...
    sortedProducts.forEach((product) => {
        ...
        tdIcon.addEventListener('click', async (e) => {
            e.stopPropagation();
            const { openProductIconPicker } = await import('./product-icon-picker.js?v=3.0.79');
...
async function openProfilePictureSettingsModal() {
    ...
    if (defaultImageChooseBtn) {
        defaultImageChooseBtn.onclick = async () => {
            const { openProfilePictureModal } = await import('./profile-picture-modal.js?v=3.0.79');
...
export function setupToolbarHistoryButton() {
    ...
    historyBtn.onclick = async (event) => {
        ...
        try {
            const { openHistorikV3 } = await import('./historik-v3.js?v=3.0.79');
            openHistorikV3();
        } catch (err) {
            console.error('Kunne ikke åbne Historik v3:', err);
            // Fallback til v2
            try {
                const { openHistorikModal } = await import('./historik-modal.js?v=3.0.79');
                openHistorikModal();
            } catch (err2) {
