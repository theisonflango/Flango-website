...
import { getCachedProfilePictureUrl, batchPreWarmProfilePictures, invalidateProfilePictureCache, getDefaultProfilePicture, getDefaultProfilePictureAsync } from '../core/profile-picture-cache.js?v=3.0.63';
import { escapeHtml } from '../core/escape-html.js?v=3.0.63';
import { openHistorikV3ForUser } from './historik-v3.js?v=3.0.63';
        ...
        <div class="uap-container">
            ...
            <div class="uap-toolbar">
                <div class="uap-search-wrap">
                    ...
                    <div class="uap-search-dropdown" id="uap-search-dropdown" style="display:none;"></div>
                </div>
...
function wireSearch() {
    ...
    const dropdown = panelEl.querySelector('#uap-search-dropdown');
    ...
    // Build dropdown items from users
    const customers = allUsers.filter(u => u.role === 'kunde').sort((a, b) => (a.name || '').localeCompare(b.name || ''));
    dropdown.innerHTML = customers.map(u =>
        `<div class="uap-search-dropdown-item" data-name="${escapeHtml(u.name || '')}">${escapeHtml(u.name || '')} <span style="opacity:0.5;">#${u.number || ''}</span></div>`
    ).join('');

    function filterDropdownItems(query) {
        const q = (query || '').toLowerCase().trim();
        dropdown.querySelectorAll('.uap-search-dropdown-item').forEach(item => {
    ...
    dropdown.addEventListener('click', (e) => {
        const item = e.target.closest('.uap-search-dropdown-item');
        if (!item) return;
        ...
        // Auto-highlight the selected user
        const filtered = getFilteredUsers();
        highlightedUserId = filtered.length > 0 ? filtered[0].id : null;
        renderActiveTab();
    });
...
function openPPLightbox(entry, mode = 'all', userRows = null) {
    ...
    function renderLightboxContent() {
        ...
        lb.querySelector('.uap-lb-activate')?.addEventListener('click', async (e) => {
            e.stopPropagation();
            const { applyProfilePicture } = await import('../core/profile-picture-utils.js?v=3.0.63');
        ...
        lb.querySelector('.uap-lb-edit')?.addEventListener('click', (e) => {
            ...
            editor.querySelector('.uap-crop-save').onclick = async () => {
                ...
                try {
                    ...
                    const { invalidateProfilePictureCache } = await import('../core/profile-picture-cache.js?v=3.0.63');
        ...
        lb.querySelector('.uap-lb-delete')?.addEventListener('click', async (e) => {
            ...
            const { removeProfilePicture } = await import('../core/profile-picture-utils.js?v=3.0.63');
