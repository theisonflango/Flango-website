import{S as je,c as st,o as ve,t as rt,f as pt,e as lt,p as Bt,u as Dt}from"./calculator-mode-BlHg1HlP.js";import{e as T,J as Ie}from"./escape-html-DenDA1C9.js";import{i as Nt,a as Pe}from"./app-B-sP5qkY.js";import{s as _e}from"./session-store-Cx51lRnK.js";import"./backend-config-D_9xaUNr.js";import"./platform-DmDvCOrB.js";import"./portal-api-ColB9ivi.js";import"./roster-DmTLAUfK.js";import"./staff-auth-DLjdrZ9g.js";import"./download-file-edyH3npu.js";import"./cafe-client-compatibility-CP9ax7CG.js";const Gt="A single centered food product icon in soft 3D clay style. Rounded puffy shapes, smooth matte clay texture, subtle soft shadows on the object only. Pastel color palette, gentle highlights, no harsh edges. Friendly, playful, child-safe aesthetic like premium mobile app UI icons. Skeuomorphic but simplified - Apple-like simplicity with minimal detail. No text, no labels, no table, no background elements. The object floats on a perfectly transparent background. Clean crisp edges suitable for UI overlay.",Vt="A single centered food product icon in Pixar-style 3D rendering. Glossy, smooth surfaces with vibrant saturated colors. Soft rounded shapes, subtle specular highlights, gentle ambient occlusion shadows on the object only. Friendly, appealing, child-safe aesthetic. Clean and simple - minimal detail, maximum charm. No text, no labels, no background elements. The object floats on a perfectly transparent background with clean crisp edges suitable for UI overlay.",Kt="3D clay-animated style rendering. Rounded puffy shapes, smooth matte clay texture, pastel color palette, gentle highlights, no harsh edges. Friendly, playful, child-safe aesthetic. Characters should look like high-quality clay figurines with warm, expressive faces.",Jt="Pixar/Dreamworks-style 3D animated rendering. Glossy, smooth surfaces with vibrant saturated colors. Soft rounded shapes, subtle specular highlights. Friendly, appealing, child-safe aesthetic with maximum charm. Characters should look like they belong in a Pixar feature film.",$e=[{emoji:"🍫",name:"Chokolade"},{emoji:"🍽️",name:"Tallerken"},{emoji:"🍷",name:"Glas"},{emoji:"🍎",name:"Æble"},{emoji:"🥜",name:"Nødder"},{emoji:"🥪",name:"Sandwich"},{emoji:"🍕",name:"Pizza"},{emoji:"🥤",name:"Sodavand"},{emoji:"🍚",name:"Ris"},{emoji:"🍣",name:"Sushi"},{emoji:"🥢",name:"Spisepinde"},{emoji:"🍞",name:"Brød"},{emoji:"🥝",name:"Kiwi"},{emoji:"🍇",name:"Vindruer"},{emoji:"🍐",name:"Pære"},{emoji:"🍉",name:"Vandmelon"},{emoji:"🍙",name:"Risbolle"},{emoji:"🍲",name:"Gryde"},{emoji:"🥘",name:"Pande"},{emoji:"🫘",name:"Bønner"},{emoji:"🍔",name:"Burger"},{emoji:"🌶️",name:"Chili"},{emoji:"🧄",name:"Hvidløg"},{emoji:"🍳",name:"Stegepande"},{emoji:"🔥",name:"Ild"},{emoji:"😋",name:"Lækkert"},{emoji:"🍰",name:"Kage"},{emoji:"♨️",name:"Varmt"},{emoji:"🍪",name:"Småkage"}],qe={reference:"📷 AI'en ser kun maden på fotoet og laver et helt nyt ikon fra bunden.",motiv:"🖼️ AI'en genskaber hele kompositionen (mad, tallerkener, personer) i valgt stil.",portrait:"🍽️ AI'en laver en animeret figur af kokken der præsenterer/holder retten."},ye=["🪄 AI'en tegner dit ikon...","🎨 Vælger de bedste farver...","✨ Tilføjer de sidste detaljer...","🖌️ Næsten klar..."],Ae=[{key:"institution",emoji:"🏠",label:"",desc:"Fra jeres ikonbibliotek",productOnly:!0},{key:"standard",emoji:"📁",label:"Standard",desc:"Flangos 3D-ikoner",productOnly:!0},{key:"emoji",emoji:"😀",label:"Emoji",desc:"Vælg en emoji som ikon",productOnly:!0},{key:"shared",emoji:"🌐",label:"Fra andre",desc:"Delte ikoner fra andre institutioner",productOnly:!0},{key:"ai",emoji:"🪄",label:"AI Ikon",desc:"Lad AI'en tegne et ikon for dig",productOnly:!1},{key:"camera",emoji:"📸",label:"Kamera",desc:"Tag foto af maden med kameraet",productOnly:!1},{key:"upload",emoji:"📤",label:"Upload",desc:"Upload eget billede som ikon",productOnly:!1}];function Yt(Ce){const{mode:ct="product",institutionId:V,productId:P,productName:Te="",currentIcon:Me,editingIcon:x,adminProfile:dt,showCustomAlert:O,playSound:fe,defaultSource:ut,onResult:M}=Ce,k=ct==="product",F=!!x;function be(e){if(!e)return;e.classList.remove("pip-disabled");const i=e.querySelector("#pip-mode-hint");i&&(i.style.display="none")}function mt(e){if(!e)return;e.classList.add("pip-disabled");const i=e.querySelector("#pip-mode-hint");i&&(i.style.display="")}const Fe=Nt();let r=F?"ai":ut||(k?"institution":"ai");r==="ai"&&!Fe&&(r=k?"institution":"camera");let v="pixar",L="reference",w=null,B=null,q=null,K=null,R=null,c=null,S=null,A=!1,p="",H=null,y=null,Z=!1,Q=!1;const Re=localStorage.getItem("flango_institution_name")||"Jeres",He=Ae.find(e=>e.key==="institution");He&&(He.label=`${Re}s ikoner`);const oe=Ae.filter(e=>["institution","standard","emoji","shared"].includes(e.key)),ze=Ae.filter(e=>["ai","camera","upload"].includes(e.key)&&(e.key!=="ai"||Fe)),vt=["upload","camera","ai"].includes(r);let E=F||vt?"create":k?"select":"create";const se=document.createElement("div");se.className="pip-overlay";const j=document.createElement("div");j.className="pip-fullscreen";const yt=F?"Redigér ikon med AI":k?"Produktikon":"Opret nyt ikon";j.innerHTML=`
    ...
    `,se.appendChild(j),document.body.appendChild(se);const _=j.querySelector("#pip-content"),J=j.querySelector("#pip-source-zone"),W=j.querySelector(".pip-save-btn");let l=null;const $=()=>se.remove();j.querySelector(".pip-cancel-btn").addEventListener("click",$),j.querySelector("#pip-header-back")?.addEventListener("click",$),j.querySelector(".pip-manage-library-btn")?.addEventListener("click",()=>{const e={...Ce};$(),typeof window.__flangoOpenIconLibrary=="function"&&window.__flangoOpenIconLibrary(()=>{Yt(e)})});function ft(){j.querySelectorAll(".pip-header-tab").forEach(e=>{e.classList.toggle("active",e.dataset.cat===E)})}j.querySelectorAll(".pip-header-tab").forEach(e=>{e.addEventListener("click",()=>{E=e.dataset.cat,r=(E==="select"?oe:ze)[0].key,p="",ft(),ee(),D(),f()})});function ee(){if(F){J.style.display="none";return}const e=E==="select"?oe:ze;J.innerHTML=`
        ...
        `,l=J.querySelector(".pip-preview-box"),l.querySelector(".pip-preview-clear"),S?l.innerHTML=S+'<button type="button" class="pip-preview-clear" title="Fjern valgt ikon">✕</button>':Me?.url&&(S=`<img src="${Me.url}"><div class="pip-preview-label">Nuværende ikon</div>`,l.innerHTML=S+'<button type="button" class="pip-preview-clear" title="Fjern valgt ikon">✕</button>'),l.querySelector(".pip-preview-clear")?.addEventListener("click",i=>{i.stopPropagation(),te()}),J.querySelectorAll(".pip-source-card").forEach(i=>{i.addEventListener("click",()=>{r===i.dataset.source?(r=null,J.querySelectorAll(".pip-source-card").forEach(s=>s.classList.remove("active"))):(r=i.dataset.source,J.querySelectorAll(".pip-source-card").forEach(s=>s.classList.toggle("active",s.dataset.source===r))),D(),f()})})}async function bt(e){if(!H){H=[];try{(await pt(V)).forEach(n=>H.push({label:n.name,path:n.icon_url,source:Re,imgSrc:n.icon_url,tags:n.tags||""}))}catch{}je.forEach(t=>H.push({label:t.label,path:t.path,source:"Standard",imgSrc:t.path})),$e.forEach(({emoji:t,name:n})=>H.push({label:n,path:null,source:"Emoji",emoji:t}));try{(await st(V)).icon_use_shared_enabled&&(await lt(V)).forEach(a=>H.push({label:a.name,path:a.icon_url,source:"Delt",imgSrc:a.icon_url,tags:a.tags||""}))}catch{}}const i=p?H.filter(t=>t.label.toLowerCase().includes(p)||t.source.toLowerCase().includes(p)||(t.tags||"").toLowerCase().includes(p)):H;if(i.length===0){e.innerHTML=p?`<div class="pip-empty-state">Ingen ikoner matcher "${T(p)}"</div>`:'<div class="pip-empty-state">Ingen ikoner fundet</div>';return}if(p){const t=document.createElement("div");t.style.cssText="font-size:13px;color:#64748b;margin-bottom:12px;",t.textContent=`${i.length} resultat${i.length===1?"":"er"}`,e.appendChild(t)}const s=document.createElement("div");s.className="pip-icon-grid",i.forEach(t=>{const n=document.createElement("div");n.className=`pip-icon-option ${c===t.path?"selected":""}`,t.emoji?(n.innerHTML=`<span style="font-size:40px">${t.emoji}</span><span>${t.source}</span>`,n.addEventListener("click",()=>{if(A&&t.path){re(t.path,t.id);return}y=t.emoji,c=null,S=`<span style="font-size:48px">${t.emoji}</span><div class="pip-preview-label">Emoji</div>`,l&&(l.innerHTML=S+'<button type="button" class="pip-preview-clear" title="Fjern valgt ikon">✕</button>'),l?.querySelector(".pip-preview-clear")?.addEventListener("click",te),f()})):(n.dataset.path=t.path,n.innerHTML=`<img src="${t.imgSrc}" alt="${t.label}"><span>${t.label}<br><small style="color:#64748b">${t.source}</small></span>`,n.addEventListener("click",()=>{if(A){re(t.path,t.id);return}c=t.path,y=null,s.querySelectorAll(".pip-icon-option").forEach(a=>a.classList.toggle("selected",a.dataset.path===t.path)),z(t.imgSrc,t.label),f()})),s.appendChild(n)}),e.appendChild(s)}async function re(e,i){A=!1;try{const t=await(await fetch(e)).blob(),n=new File([t],"ref.webp",{type:t.type||"image/webp"}),a=await ve(n);E="create",r="ai",ee(),D(),setTimeout(()=>{Oe(a),B=i||null},50)}catch(s){console.error("[referencePickComplete]",s),A=!1}}function z(e,i){l&&(e?(S=`<img src="${e}">`+(i?`<div class="pip-preview-label">${i}</div>`:""),l.innerHTML=S+'<button type="button" class="pip-preview-clear" title="Fjern valgt ikon">✕</button>',l.querySelector(".pip-preview-clear")?.addEventListener("click",te)):te(),l.classList.remove("generating"))}function te(){S=null,c=null,y=null,q=null,l&&(l.innerHTML='<span class="pip-preview-placeholder">❓</span><div class="pip-preview-label">Intet valgt</div>'),f()}function gt(){l&&(l.innerHTML='<span class="pip-preview-placeholder">🪄</span><div class="pip-preview-label">Genererer...</div>',l.classList.add("generating"))}function ht(){l&&(l.classList.remove("generating"),l.innerHTML='<span class="pip-preview-placeholder">⚠️</span><div class="pip-preview-label">Mislykkedes</div>')}function f(){let e=!1;r===null?e=!!(c||y):r==="standard"?e=!!c:r==="emoji"?e=!!y:r==="institution"||r==="shared"?e=!!c:r==="upload"?e=!!K:r==="camera"?e=!!R:r==="ai"&&(e=!!q),W.disabled=!e}W.addEventListener("click",async()=>{if(r===null){y?(M({type:"emoji",emoji:y}),$()):c&&(M({type:"icon",emoji:`${Ie}${c}`,url:c}),$());return}if(r==="standard")c&&M({type:"standard",emoji:`${Ie}${c}`,url:c}),$();else if(r==="emoji")y&&M({type:"emoji",emoji:y}),$();else if(r==="institution"||r==="shared")c&&M({type:"icon",emoji:`${Ie}${c}`,url:c}),$();else if(r==="upload"||r==="camera"){const e=r==="upload"?K:R;if(!e)return;W.disabled=!0,W.textContent="Gemmer...";try{const i=dt?.user_id;if(!i)throw new Error("Admin bruger ID ikke fundet");const s=await ve(e),t=P||crypto.randomUUID(),n=new File([s],`${t}.webp`,{type:"image/webp"}),a=await Dt(n,V,t,i);if(a.success)fe?.("success"),M({type:"upload",url:a.icon_signed_url||a.icon_url,storagePath:a.icon_storage_path,updatedAt:a.icon_updated_at}),$();else throw new Error(a.error||"Upload fejlede")}catch(i){console.error("[pip save]",i),O?.("Fejl",i.message),W.textContent=k?"Vælg":"Gem ikon",f()}}else r==="ai"&&q&&(M({type:"ai",url:q,metadata:{style:v,photoMode:L}}),$())});function D(){if(_.innerHTML="",A){const e=document.createElement("div");e.style.cssText="padding:10px 16px;background:rgba(124,58,237,0.15);border:1px solid rgba(124,58,237,0.3);border-radius:10px;margin-bottom:14px;display:flex;align-items:center;gap:10px;font-size:13px;color:#c4b5fd;",e.innerHTML=`<span style="font-size:20px">🖼️</span> <span>Vælg et ikon som foto-reference til AI — klik på det ikon du vil bruge</span>
                <button type="button" style="margin-left:auto;background:none;border:1px solid rgba(124,58,237,0.4);border-radius:6px;color:#a78bfa;padding:4px 12px;cursor:pointer;font-size:12px;font-weight:600;" id="pip-cancel-pick">Annuller</button>`,_.appendChild(e),e.querySelector("#pip-cancel-pick").addEventListener("click",()=>{A=!1,E="create",r="ai",ee(),D()})}if(E==="select"){const e=document.createElement("div");e.className="pip-search-row",e.innerHTML=`<input type="text" class="pip-search-input" id="pip-search" placeholder="🔍 Søg ${r?"i "+(oe.find(n=>n.key===r)?.label||""):"i alle ikoner"}..." value="${T(p)}" autocomplete="off"><button type="button" class="pip-search-clear" id="pip-search-clear" ${p?"":'style="display:none"'}>✕</button>`,_.appendChild(e);let i=null;const s=e.querySelector("#pip-search"),t=e.querySelector("#pip-search-clear");s.addEventListener("input",()=>{clearTimeout(i),t.style.display=s.value?"":"none",i=setTimeout(()=>{p=s.value.trim().toLowerCase(),ge()},250)}),t.addEventListener("click",()=>{p="",s.value="",t.style.display="none",s.focus(),ge()}),s.focus()}ge()}function ge(){const e=_.querySelector(".pip-content-section");e&&e.remove();const i=document.createElement("div");if(i.className="pip-content-section",E==="select"&&r===null)bt(i);else switch(r){case"standard":kt(i);break;case"emoji":St(i);break;case"institution":wt(i);break;case"shared":xt(i);break;case"upload":Et(i);break;case"camera":jt(i);break;case"ai":It(i);break}_.appendChild(i)}function kt(e){const i=document.createElement("div");i.className="pip-icon-grid";const s=p?je.filter(t=>t.label.toLowerCase().includes(p)):je;if(s.length===0){e.innerHTML=`<div class="pip-empty-state">Ingen standard-ikoner matcher "${T(p)}"</div>`;return}s.forEach(t=>{const n=document.createElement("div");n.className=`pip-icon-option ${c===t.path?"selected":""}`,n.dataset.path=t.path,n.innerHTML=`<img src="${t.path}" alt="${t.label}"><span>${t.label}</span>`,n.addEventListener("click",()=>{if(A){re(t.path,t.id);return}c=t.path,y=null,e.querySelectorAll(".pip-icon-option").forEach(a=>a.classList.toggle("selected",a.dataset.path===t.path)),z(t.path,t.label),f()}),i.appendChild(n)}),e.appendChild(i)}function St(e){const i=p?$e.filter(n=>n.name.toLowerCase().includes(p)||n.emoji.includes(p)):$e;e.innerHTML=`
        ...
        `,i.length===0&&(e.querySelector("#pip-emoji-grid").innerHTML=`<div class="pip-empty-state">Ingen emojis matcher "${T(p)}"</div>`);const s=e.querySelector("#pip-emoji-input"),t=e.querySelector("#pip-emoji-grid");i.forEach(({emoji:n,name:a})=>{const d=document.createElement("div");d.className="pip-icon-option",d.innerHTML=`<span style="font-size:36px">${n}</span><span>${a}</span>`,d.addEventListener("click",()=>{y=n,c=null,s.value=n,S=`<span style="font-size:48px">${n}</span><div class="pip-preview-label">${a}</div>`,l&&(l.innerHTML=S+'<button type="button" class="pip-preview-clear" title="Fjern valgt ikon">✕</button>'),l?.querySelector(".pip-preview-clear")?.addEventListener("click",te),f()}),t.appendChild(d)}),s.addEventListener("input",()=>{y=s.value||null,c=null,y&&(S=`<span style="font-size:48px">${y}</span><div class="pip-preview-label">Emoji</div>`,l&&(l.innerHTML=S)),f()})}function wt(e){e.innerHTML='<div class="pip-empty-state">Indlæser...</div>',Ue(e,()=>pt(V),"Ingen egne ikoner endnu — upload eller generer via AI.")}function xt(e){e.innerHTML='<div class="pip-empty-state">Indlæser...</div>',Lt(e)}async function Lt(e){if(e||(e=_.querySelector(".pip-content-section")),!e)return;if(!(await st()).icon_use_shared_enabled){e.innerHTML='<div class="pip-empty-state">Brug af delte ikoner er ikke aktiveret. Slå det til under Institutionsindstillinger.</div>';return}Ue(e,()=>lt(),"Ingen delte ikoner tilgængelige endnu.")}async function Ue(e,i,s){let t=await i();if(e.innerHTML="",p&&(console.log("[loadIconGrid] Searching for:",p,"in",t.length,"icons. Sample tags:",t.slice(0,3).map(a=>a.name+":"+(a.tags||"none"))),t=t.filter(a=>a.name.toLowerCase().includes(p)||(a.tags||"").toLowerCase().includes(p)),console.log("[loadIconGrid] After filter:",t.length,"matches")),t.length===0){e.innerHTML=p?`<div class="pip-empty-state">Ingen ikoner matcher "${T(p)}"</div>`:`<div class="pip-empty-state">${s}</div>`;return}const n=document.createElement("div");n.className="pip-icon-grid",t.forEach(a=>{const d=document.createElement("div");d.className=`pip-icon-option ${c===a.icon_url?"selected":""}`,d.dataset.path=a.icon_url;const m=a.source==="uploaded"?"📤":"🪄";d.innerHTML=`<img src="${a.icon_url}" alt="${a.name}"><span>${m} ${a.name}</span>`,d.addEventListener("click",()=>{if(A){re(a.icon_url,a.id);return}c=a.icon_url,y=null,n.querySelectorAll(".pip-icon-option").forEach(N=>N.classList.toggle("selected",N.dataset.path===a.icon_url)),z(a.icon_url,a.name),f()}),n.appendChild(d)}),e.appendChild(n)}function Et(e){e.innerHTML=`
            ...
            </div>`;const i=e.querySelector("#pip-cam-trigger"),s=e.querySelector("#pip-cam-preview"),t=e.querySelector("#pip-cam-img"),n=async()=>{try{const a=await rt({showCustomAlert:O});a&&(R=a,t.src=URL.createObjectURL(a),s.style.display="",i.style.display="none",z(t.src,"Foto"),f())}catch(a){console.error("[camera]",a)}};i.addEventListener("click",n),e.querySelector("#pip-cam-retake").addEventListener("click",()=>{R=null,s.style.display="none",i.style.display="",z(null),f(),n()}),e.querySelector("#pip-cam-use").addEventListener("click",()=>{R&&(K=R,r="camera",f())}),e.querySelector("#pip-cam-ai").addEventListener("click",()=>{if(!R)return;const a=R;r="ai",sourceCards.forEach(d=>d.classList.toggle("active",d.dataset.source==="ai")),D(),setTimeout(()=>Oe(a),50)})}function It(e){const i=T(Te||x?.name||"");e.innerHTML=`
            <!-- Step 1: Product name -->
        ...
        `,Pt(e)}function Pt(e){const i=e.querySelector("#pip-ai-name"),s=e.querySelectorAll(".pip-style-card"),t=e.querySelector("#pip-custom-section"),n=e.querySelector("#pip-custom-prompt"),a=e.querySelector("#pip-ai-step1"),d=e.querySelector("#pip-photo-actions"),m=e.querySelector("#pip-photo-preview"),N=e.querySelector("#pip-photo-img"),Be=e.querySelector("#pip-photo-remove"),De=e.querySelector("#pip-photo-file"),Ne=De?.querySelector("input"),_t=e.querySelector("#pip-photo-camera"),Y=e.querySelector("#pip-mode-section"),he=e.querySelectorAll(".pip-mode-card"),Ge=e.querySelector("#pip-mode-desc"),pe=e.querySelector("#pip-generate"),Ve=e.querySelector("#pip-loading"),Ke=e.querySelector("#pip-loading-msg"),ke=e.querySelector("#pip-result"),$t=e.querySelector("#pip-result-img"),qt=e.querySelector("#pip-result-label"),At=e.querySelector("#pip-retry"),ie=e.querySelector("#pip-error"),Je=e.querySelector("#pip-error-msg"),Ct=e.querySelector("#pip-error-retry"),le=e.querySelector("#pip-advanced"),ce=e.querySelector("#pip-style-prompt"),de=e.querySelector("#pip-photo-prompt"),Tt=e.querySelector("#pip-reset-prompts");s.forEach(o=>{o.addEventListener("click",()=>{v=o.dataset.style,s.forEach(I=>I.classList.toggle("active",I===o));const b=v==="custom";if(t.style.display=b?"block":"none",a.style.display=b?"none":"block",Y){const I=b||!w;Y.classList.toggle("pip-disabled",I);const X=Y.querySelector("#pip-mode-hint");X&&(X.style.display=I?"":"none")}U()})}),De?.addEventListener("click",()=>Ne?.click()),Ne?.addEventListener("change",o=>{o.target.files[0]&&Ye(o.target.files[0]),o.target.value=""}),e.querySelector("#pip-photo-pick")?.addEventListener("click",()=>{A=!0,E="select",r=oe[0].key,ee(),D()}),_t?.addEventListener("click",async()=>{try{const o=await rt({showCustomAlert:O});o&&Ye(o)}catch(o){console.error("[aiPhotoCamera]",o)}}),Be?.addEventListener("click",Ft);async function Ye(o){try{B=null,w=await ve(o),Mt(w)}catch(b){console.error("[aiPhoto]",b)}}function Mt(o){N.src=URL.createObjectURL(o),m.style.display="flex",d.style.display="none",v!=="custom"&&be(Y),U()}function Ft(){w=null,B=null,m.style.display="none",d.style.display="flex",mt(Y),U()}he.forEach(o=>{o.addEventListener("click",()=>{if(o.dataset.mode==="portrait"&&!Pe()){O?.("Ikke tilgængelig","Mad Portræt kan aktiveres af superadmin.");return}L=o.dataset.mode,he.forEach(b=>b.classList.toggle("active",b===o)),Ge.textContent=qe[L]||"",U()})});function Rt(){return v==="custom"?"":L==="portrait"&&!!w?v==="pixar"?Jt:Kt:v==="pixar"?Vt:Gt}function Ht(){const o=i?.value?.trim()||"";return v==="custom"?"":w?L==="portrait"?`Transform into ${v==="clay"?"clay-animated":"Pixar/Dreamworks-style animated"} version. Person (cook/chef) proudly presenting food. Preserve features. Include food prominently.${o?` Dish: ${o}`:""}`:L==="motiv"?`Recreate composition from photo as 3D icon. Keep food arrangement, render in ${v==="pixar"?"glossy Pixar":"Flango clay"} style. If people appear, include as stylized characters.${o?` Product: ${o}`:""}`:`Use photo only to identify food. Create fresh ${v==="pixar"?"Pixar-style":"clay-style"} icon. Ignore background/angle. Include container only if needed.${o?` Product: ${o}`:""}`:`The food item is: ${o}
Include a bowl, plate, cup, or container only if the food needs one.`}function U(){le?.open&&(!Z&&ce&&(ce.value=Rt()),!Q&&de&&(de.value=Ht()))}le?.addEventListener("toggle",()=>{le.open&&(Z=!1,Q=!1,U())}),ce?.addEventListener("input",()=>{Z=!0}),de?.addEventListener("input",()=>{Q=!0}),Tt?.addEventListener("click",()=>{Z=!1,Q=!1,U()}),i?.addEventListener("input",U);let Se=!1,Xe=null;async function we(){if(Se)return;const o=v==="custom",b=i?.value?.trim()||"",I=n?.value?.trim()||"",X=!!w;if(o&&!I){O?.("Manglende prompt","Skriv en prompt i tekstfeltet");return}if(!o&&!b&&!X){O?.("Manglende input","Skriv et produktnavn eller upload et foto");return}Se=!0,pe.style.display="none",ke.style.display="none",ie&&(ie.style.display="none"),Ve.style.display="block",gt(),q=null;let Ze=!1,xe=0;Ke.textContent=ye[0],Xe=setInterval(()=>{xe=(xe+1)%ye.length,Ke.textContent=ye[xe]},3e3);try{const{data:{session:ue}}=await _e.auth.getSession(),ae=ue?.access_token||"",Qe=_e.supabaseUrl||_e.rest?.url?.replace("/rest/v1","")||"",ne=le?.open&&(Z||Q),We=ne?((ce?.value?.trim()||"")+`

`+(de?.value?.trim()||"")).trim():"",et={Authorization:`Bearer ${ae}`};let g;const tt=h=>{B?h.append("reference_icon_id",B):h.append(k?"reference_image":"photo",w,"photo.webp")};if(X&&!o&&!ne)g=new FormData,k&&P?g.append("product_id",P):g.append("save_to_library_only","true"),g.append("product_name",b),tt(g),g.append("style",v),g.append("photo_mode",L);else if(X)g=new FormData,k&&P?g.append("product_id",P):g.append("save_to_library_only","true"),g.append("product_name",b),tt(g),g.append("prompt_mode","custom"),g.append("custom_prompt",ne?We:I);else{et["Content-Type"]="application/json";const h={product_name:b,style:o?"custom":v};k&&P?h.product_id=P:h.save_to_library_only=!0,(o||ne)&&(h.prompt_mode="custom",h.custom_prompt=ne?We:I||b),g=JSON.stringify(h)}const u=await(await fetch(`${Qe}/functions/v1/generate-product-icon`,{method:"POST",headers:et,body:g})).json();if(!u.success)throw new Error(u.error||"Generering fejlede");if(!u.image_base64)throw new Error("Ingen billede-data fra serveren");const Le=atob(u.image_base64),it=new Uint8Array(Le.length);for(let h=0;h<Le.length;h++)it[h]=Le.charCodeAt(h);const at=new Blob([it],{type:`image/${u.format||"webp"}`}),nt=await Bt(at);console.log(`[AI Generate] Raw ${(at.size/1024).toFixed(1)}KB → Compressed ${(nt.size/1024).toFixed(1)}KB`);const zt=u.fallback_name||b||"AI ikon",C=new FormData;C.append("file",new File([nt],"ai-icon.webp",{type:"image/webp"})),C.append("institutionId",V),k&&P?C.append("productId",P):C.append("skipProduct","true"),C.append("libraryName",zt),u.style&&C.append("aiStyle",String(u.style)),u.mode&&C.append("aiPhotoMode",String(u.mode)),u.prompt_mode&&C.append("aiPromptMode",String(u.prompt_mode));const Ee=await fetch(`${Qe}/functions/v1/upload-product-icon`,{method:"POST",headers:{Authorization:`Bearer ${ae}`},body:C}),G=await Ee.json().catch(()=>({}));if(!Ee.ok||!G?.success)throw new Error(G?.error||`Upload fejlede (${Ee.status})`);const ot=G.icon_signed_url||G.icon_url||null,Ut=G.icon_storage_path||null,Ot=G.library_icon_url||null;q=ot||Ot,$t.src=q,ke.style.display="block";const me=[];if(u.style&&me.push(u.style==="clay"?"🏺 Clay":u.style==="pixar"?"🎬 Pixar":"✍️ Fri prompt"),u.mode){const h={"photo-reference":"📷 Reference","photo-motiv":"🖼️ Motiv","photo-avatar":"👤 Avatar","photo-portrait":"🍽️ Mad Portræt",text:"✏️ Tekst","custom-photo":"🎨 Fri prompt","custom-text":"🎨 Fri prompt"};me.push(h[u.mode]||u.mode)}qt.textContent=me.join(" · "),z(q,me.join(" · ")),f(),fe?.("success"),k&&P&&M({type:"ai",url:ot,storagePath:Ut,updatedAt:G.icon_updated_at,metadata:{style:v,photoMode:L}})}catch(ue){console.error("[AI Generate]",ue),Ze=!0;const ae=ue?.message||"Generering fejlede";ie&&Je?(Je.textContent=ae,ie.style.display="flex"):O?.("AI Fejl",ae),ht(),fe?.("error")}finally{clearInterval(Xe),Ve.style.display="none",!q&&!Ze&&(pe.style.display="block"),Se=!1}}pe?.addEventListener("click",we),At?.addEventListener("click",()=>{ke.style.display="none",pe.style.display="block",we()}),Ct?.addEventListener("click",()=>{ie.style.display="none",we()}),F&&x&&(L=Pe()?"portrait":"reference",he.forEach(o=>o.classList.toggle("active",o.dataset.mode===L)),Ge.textContent=qe[L]||"",Be.style.display="none",d.style.display="none",N.src=x.icon_url,m.style.display="flex",be(Y),B=x.id||null,fetch(x.icon_url).then(o=>o.blob()).then(o=>{w=new File([o],"ref.webp",{type:"image/webp"}),U()}).catch(o=>console.error("[fetchEditIcon]",o)))}function Oe(e){w=e,B=null;const i=_.querySelector("#pip-photo-preview"),s=_.querySelector("#pip-photo-img"),t=_.querySelector("#pip-photo-actions"),n=_.querySelector("#pip-mode-section");s&&(s.src=URL.createObjectURL(e)),i&&(i.style.display="flex"),t&&(t.style.display="none"),n&&v!=="custom"&&be(n)}ee(),D()}export{Yt as openProductIconPicker};
