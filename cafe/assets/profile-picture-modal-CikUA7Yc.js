import{g as he,b as le,p as Z,u as ne,s as se,f as xe,c as _e,d as ke,e as pe,A as we,h as Le}from"./app-CVYFbey3.js";import{r as Se}from"./wallet-balances-3RttQEZA.js";import{e as h}from"./escape-html-CPboFElj.js";import{s as R}from"./config-and-supabase-C9IyeGNd.js";import{f as Ae}from"./calculator-mode-DsI3V8X_.js";import{a as ae}from"./cafe-client-compatibility-BKR5-Ls7.js";import"./platform-qyiS8StQ.js";import"./session-store-DLvMKKKS.js";import"./download-file-_YNye86Y.js";function Ee(e){const t=!!e?.profile_pictures_ai_enabled;return{masterOn:t,usable:t&&e?.ai_provider_openai!==!1}}async function Ve(e,t={}){const{onSaved:b,showCustomAlert:k}=t,c=window.__flangoGetInstitutionById?.(e.institution_id);if(!c)return;let g=c.profile_picture_types||["upload","camera","library"];const o=e.id!=="__default__"&&await he(c),n=document.createElement("div");n.className="profile-pic-modal-overlay";const r=document.createElement("div");r.className="profile-pic-modal";const y=e.number?` (${e.number})`:"";r.innerHTML=`
    ...
    `).join("");e.innerHTML=`
        ...
        <div class="profile-pic-subview">
            ...
            </div>`}).join("");v?w.innerHTML=v:w.innerHTML='<div style="color:#94a3b8;font-size:11px;padding:8px;">Ingen eksisterende billeder. Brug kamera eller upload ovenfor.</div>',w.querySelectorAll("[data-lib-index]").forEach(a=>{a.addEventListener("click",async()=>{const S=parseInt(a.dataset.libIndex),_=i[S];if(_){a.querySelector("img").style.borderColor="#f59e0b";try{const L=p.get(_.id)||_.storage_path,j=await(await fetch(L)).blob();w.style.display="none",V(j,"Eksisterende billede sendes til AI og slettes straks efter","library")}catch{a.querySelector("img").style.borderColor="#f87171",setTimeout(()=>{a.querySelector("img").style.borderColor="transparent"},1500)}}})})})}export{Ve as openProfilePictureModal};
