import{g as he,b as le,p as Z,u as ne,s as pe,f as xe,d as _e,e as ke,h as se,A as we,j as Le,k as Se}from"./app-Vn0WZW6u.js";import{r as Ae}from"./roster-DkJnjOyG.js";import{e as h}from"./escape-html-VWVHoSZX.js";import{s as B}from"./session-store-BxvIzIj5.js";import{f as Ee}from"./calculator-mode-DIFniJrO.js";import{a as ae}from"./backend-config-D_9xaUNr.js";import"./platform-B9jbD3g4.js";import"./staff-auth-DLjdrZ9g.js";import"./portal-api-D5MzO3nJ.js";import"./download-file-bWQBCA8D.js";import"./cafe-client-compatibility-BvTeuWUS.js";function $e(e){const t=e?.media||{},b=!!t.profile_pictures_ai_enabled;return{masterOn:b,usable:b&&t.ai_provider_openai!==!1}}async function Ne(e,t={}){const{onSaved:b,showCustomAlert:k}=t,c=window.__flangoGetInstitutionById?.(e.institution_id);if(!c)return;let g=c.media?.profile_picture_types||["upload","camera","library"];const o=e.id!=="__default__"&&await he(c),n=document.createElement("div");n.className="profile-pic-modal-overlay";const r=document.createElement("div");r.className="profile-pic-modal";const y=e.number?` (${e.number})`:"";r.innerHTML=`
    ...
    `,n.appendChild(r),document.body.appendChild(n);const l=()=>{M(),n.remove()};r.querySelector(".profile-pic-modal-close").addEventListener("click",l),n.addEventListener("click",p=>{p.target===n&&l()});const d=r.querySelector("#pp-current-section");await ce(d,e);const m=document.createElement("div");m.className="profile-pic-permissions",m.style.cssText="padding:8px 16px;display:flex;flex-wrap:wrap;gap:6px;align-items:center;font-size:12px;color:#94a3b8;",m.innerHTML='<span style="font-weight:600;margin-right:4px;">Tilladelser:</span>';const $=$e(c),U=[{label:"Manuel upload",optOut:e.profile_picture_opt_out_upload},{label:"Aula",optOut:e.profile_picture_opt_out_aula},{label:"Kamera",optOut:e.profile_picture_opt_out_camera},{label:"AI-Avatar",optOut:e.profile_picture_opt_out_ai||!$.usable}];for(const p of U){const E=document.createElement("span");E.style.cssText=`padding:2px 8px;border-radius:6px;font-size:11px;font-weight:600;${p.optOut?"background:rgba(239,68,68,0.15);color:#ef4444;":"background:rgba(34,197,94,0.15);color:#22c55e;"}`,E.textContent=`${p.optOut?"❌":"✅"} ${p.label}`,m.appendChild(E)}r.querySelector("#pp-current-section").after(m);const P=r.querySelector("#pp-type-grid"),F=[{key:"upload",icon:"📁",label:"Manuel upload",optOutField:"profile_picture_opt_out_upload"},{key:"camera",icon:"📷",label:"Kamera",optOutField:"profile_picture_opt_out_camera"},{key:"library",icon:"🎨",label:"Bibliotek"},{key:"icons",icon:"🖼️",label:"Ikoner"},{key:"ai_avatar",icon:"🤖",label:"AI-Avatar",requiresAi:!0,optOutField:"profile_picture_opt_out_ai"},...o?[{key:"flango_avatar",asset:"Icons/webp/Avatar/v2/characters/mango-001/rolig.webp",label:"Avatarbibliotek"}]:[]],f=r.querySelector("#pp-subview");let A=null;function M(){A&&(A.getTracks().forEach(p=>p.stop()),A=null)}let w=!1;const R=t.preSelectType||null,H=t.referenceImageUrl||null,D=(p,E)=>{const x=document.createElement("button");x.className="profile-pic-type-btn pp-type-disabled",x.title=E,x.innerHTML=`${X(p)}${p.label}<div class="pp-type-disabled-reason">${h(E)}</div>`,x.style.cssText="opacity:0.4;pointer-events:none;position:relative;",P.appendChild(x)},q=()=>{r.classList.remove("is-flango-avatar-library"),r.querySelector(".profile-pic-modal-header h3").textContent=`Profilbillede — ${e.name}${y}`};for(const p of F){if(!["icons","ai_avatar","flango_avatar"].includes(p.key)&&!g.includes(p.key))continue;if(p.requiresAi&&!$.usable){D(p,"Ikke aktiveret");continue}const E=p.optOutField&&e[p.optOutField],x=document.createElement("button");x.className="profile-pic-type-btn"+(E?" pp-type-disabled":""),E?(x.innerHTML=`${X(p)}${p.label}<div class="pp-type-disabled-reason">Fravalgt af forælder</div>`,x.style.cssText="opacity:0.4;pointer-events:none;position:relative;"):(x.innerHTML=`${X(p)}${p.label}`,x.addEventListener("click",()=>{M(),P.querySelectorAll(".profile-pic-type-btn").forEach(z=>z.classList.remove("active")),x.classList.add("active"),p.key,q(),p.key==="upload"?Te(f,e,c,l,b):p.key==="camera"?Ue(f,e,c,l,b,z=>{A=z}):p.key==="library"?Me(f,e,l,b):p.key==="icons"?He(f,e,l,b):p.key==="ai_avatar"?Re(f,e,c,l,b,z=>{A=z}):p.key==="flango_avatar"&&qe({modal:r,subview:f,user:e,numberStr:y,currentSection:d,onSaved:b,onBack:()=>{q(),x.classList.remove("active"),f.innerHTML=""}})})),P.appendChild(x),!w&&R&&p.key===R&&!E&&(w=!0,setTimeout(()=>x.click(),100))}R==="ai_avatar"&&H&&(window.__ppAiReferenceUrl=H)}function X(e){return e.asset?`<span class="type-icon type-icon-image"><img src="${h(e.asset)}" alt=""></span>`:`<span class="type-icon">${e.icon}</span>`}function qe({modal:e,subview:t,user:b,numberStr:k,currentSection:c,onSaved:g,onBack:o}){e.classList.add("is-flango-avatar-library"),e.querySelector(".profile-pic-modal-header h3").textContent=`Avatarbibliotek — ${b.name}${k}`,t.innerHTML=`
    ...
    `,t.querySelector(".profile-pic-flango-back")?.addEventListener("click",o),_e({root:t.querySelector(".profile-pic-flango-root"),userId:b.id,api:ke,managedUserName:b.name,allowRelease:!0,onAvatarChanged:async(r,y={})=>{if(!["claimed","released"].includes(y.reason))return;const l=r?.assignment?{profile_picture_url:r.assignment.image_path,profile_picture_type:"library",profile_picture_updated_at:new Date().toISOString()}:{profile_picture_url:null,profile_picture_type:null,profile_picture_updated_at:new Date().toISOString()};Object.assign(b,l),se(b.id),await ce(c,b),g?.(l)}}).open()}async function ce(e,t){if(!(t.profile_picture_url&&!t.profile_picture_opt_out)){e.innerHTML=`
        ...
        </div>`;const c=await le(t),g=e.querySelector("#pp-current-img-wrap");if(c&&g){const o=document.createElement("img");o.src=c,o.alt="",o.className="profile-pic-current-img",g.replaceWith(o)}}function Te(e,t,b,k,c){e.innerHTML=`
        ...
        <div class="profile-pic-subview">
            ...
            <div id="pp-camera-status" class="profile-pic-loading" style="text-align:center;margin-top:8px;">
                ...
                </div>`,r.querySelector("#pp-camera-save").addEventListener("click",async()=>{r.innerHTML='<div class="profile-pic-loading"><span class="profile-pic-spinner"></span> Uploader...</div>';const w=await ne(A,t.institution_id,t.id,"camera");URL.revokeObjectURL(M),w.success?(t.profile_picture_url=w.storagePath||`${t.institution_id}/${t.id}.webp`,t.profile_picture_type="camera",c&&c({profile_picture_url:t.profile_picture_url,profile_picture_type:"camera"}),k()):r.innerHTML=`<div style="color:#f87171;text-align:center;">${h(w.error||"Upload fejlede")}</div>`}),r.querySelector("#pp-camera-retry").addEventListener("click",()=>{URL.revokeObjectURL(M),r.style.display="none",l.style.display="block",n.parentElement.style.display="flex"})},"image/jpeg",.9)})}function Me(e,t,b,k){let c=null;const g=we.map((r,y)=>`
    ...
    `).join("");e.innerHTML=`
        ...
        <div class="profile-pic-subview">
        ...
        </div>`;const f=e.querySelector("#pp-ai-preview"),A=e.querySelector("#pp-ai-options"),M=e.querySelector("#pp-ai-camera-section"),w=e.querySelector("#pp-ai-source-grid"),R=e.querySelector("#pp-ai-file-input"),H=e.querySelector("#pp-ai-methods");e.querySelector("#pp-ai-method-camera")?.addEventListener("click",ye),e.querySelector("#pp-ai-method-upload")?.addEventListener("click",()=>R.click());const D=e.querySelectorAll(".pp-ai-preset-btn"),q=e.querySelector("#pp-ai-prompt-textarea");D.forEach(i=>{i.addEventListener("click",()=>{const s=Y.find(u=>u.key===i.dataset.preset);s&&(o=s,D.forEach(u=>{const v=u.dataset.preset===s.key;u.style.borderColor=v?"#f59e0b":"rgba(255,255,255,0.1)",u.style.background=v?"rgba(245,158,11,0.1)":"transparent",u.style.color=v?"#f59e0b":"#94a3b8"}))})});const p=e.querySelector("#pp-ai-advanced-section"),E=e.querySelector("#pp-ai-advanced-toggle"),x=e.querySelector("#pp-ai-adv-arrow"),z=e.querySelector("#pp-ai-prompt-section"),de=e.querySelector("#pp-ai-reset-prompt"),O=e.querySelector("#pp-ai-load-prompt");async function ee(){if(!(!$||!q))try{const{data:{session:i}}=await B.auth.getSession(),s=i?.access_token;if(!s)return;O&&(O.disabled=!0,O.textContent="⏳ Henter…");const u=new FormData;u.append("preview_prompt","true"),u.append("target_user_id",t.id),u.append("user_id",t.id),u.append("ai_style",o.key),u.append("ai_provider","openai"),u.append("hat_enabled",d?"true":"false"),u.append("hero_enabled",m?"true":"false");const a=await(await fetch(`${ae}/functions/v1/generate-profile-avatar`,{method:"POST",headers:{Authorization:`Bearer ${s}`},body:u})).json();a?.success&&typeof a.prompt=="string"&&(q.value=a.prompt,r=a.prompt)}catch(i){console.warn("[profile-picture-modal] kunne ikke indlæse aktuel prompt:",i?.message||i)}finally{O&&(O.disabled=!1,O.textContent="👁 Indlæs aktuel prompt")}}Ie().then(i=>{$=i,p&&i&&(p.style.display="")}),E?.addEventListener("click",()=>{$&&(n=!n,x&&(x.style.transform=n?"rotate(90deg)":""),z&&(z.style.display=n?"block":"none"),n&&q&&!q.value.trim()&&ee())}),O?.addEventListener("click",ee),q?.addEventListener("input",()=>{r=q.value}),de?.addEventListener("click",()=>{r="",q&&(q.value="")});const te=e.querySelector("#pp-ai-hat-checkbox"),W=e.querySelector("#pp-ai-hat-toggle");te?.addEventListener("change",()=>{d=te.checked,W&&(W.style.borderColor=d?"#f59e0b":"rgba(255,255,255,0.1)",W.style.background=d?"rgba(245,158,11,0.08)":"transparent")});const I=e.querySelector("#pp-ai-hero-checkbox"),T=e.querySelector("#pp-ai-hero-toggle");function G(i){if(!T)return;T.style.opacity="0.55",T.style.cursor="not-allowed",I&&(I.disabled=!0,I.checked=!1);const s=T.querySelector("div:last-child > div:last-child");s&&(s.textContent=i||"Kun for legendariske ekspedienter")}function fe(){if(!T)return;T.style.opacity="",T.style.cursor="pointer",I&&(I.disabled=!1);const i=T.querySelector("div:last-child > div:last-child");i&&(i.textContent="Tilføjer superhelte-tema til avataren")}G("Tjekker hero-status …"),(async()=>{try{const{data:i,error:s}=await B.rpc("get_hero_status",{p_user_id:t.id});if(s){console.warn("[profile-picture-modal] get_hero_status fejl:",s.message),G("Kunne ikke verificere status — prøv igen");return}if(P=i||null,U=!!(i&&i.qualified),U)fe();else{const u=Number(i?.sales_count??0),v=Number(i?.sales_required??500),a=Number(i?.minutes_worked??0),S=Number(i?.minutes_required??1800),_=Math.floor(a/60),L=Math.floor(S/60);G(`🔒 Kræver "legendarisk ekspedient" — pt. ${u}/${v} salg eller ${_}/${L} timer`)}}catch(i){console.warn("[profile-picture-modal] hero-status undtagelse:",i?.message||i),G("Kunne ikke verificere status — prøv igen")}})(),I?.addEventListener("change",()=>{if(I.disabled){I.checked=!1;return}m=I.checked,T&&(T.style.borderColor=m?"#8b5cf6":"rgba(255,255,255,0.1)",T.style.background=m?"rgba(139,92,246,0.08)":"transparent")});function V(i,s,u){y=i,l=u||"library",M.style.display="none",w.style.display="none",H&&(H.style.display="none"),f.style.display="block",A.style.display="block";const v=URL.createObjectURL(i),a=()=>{URL.revokeObjectURL(v),y=null,f.style.display="none",A.style.display="none",w.style.display="flex",H&&(H.style.display="flex")};if(b.media?.profile_pictures_ai_enabled!==!0){f.innerHTML=`
                <div class="profile-pic-preview-container">
                    <img src="${v}" alt="Reference" class="profile-pic-preview-img" style="border-color:rgba(245,158,11,0.4);">
                    <div style="font-size:11px;color:#94a3b8;text-align:center;">${h(s)}</div>
            ...
            </div>`,f.querySelector("#pp-ai-generate").addEventListener("click",()=>ue(i,v)),f.querySelector("#pp-ai-change-ref").addEventListener("click",a)}async function ue(i,s){f.innerHTML=`<div class="profile-pic-loading" style="padding:30px;text-align:center;">
            ...
            <div style="margin-top:8px;font-size:13px;">Genererer avatar via Microsoft Azure (EU)... (15-90 sek)</div>
        </div>`;try{const{data:{session:u}}=await B.auth.getSession(),v=u?.access_token;if(!v)throw new Error("Ikke logget ind");if(m&&!U){f.innerHTML=`<div style="padding:14px;border-radius:8px;background:rgba(239,68,68,0.1);color:#ef4444;font-size:13px;">
                    Hero-stilen kræver enten admin-rolle eller status "legendarisk ekspedient" (500 salg eller 30 timer).
                </div>`;return}const a=new FormData;a.append("target_user_id",t.id),a.append("user_id",t.id),a.append("photo",new File([i],"photo.jpg",{type:"image/jpeg"})),a.append("reference_source",l||"library"),a.append("ai_style",o.key),a.append("ai_provider","openai"),a.append("hat_enabled",d?"true":"false"),a.append("hero_enabled",m?"true":"false");const S=(r||"").trim();if(S&&$&&a.append("custom_prompt",S),d)try{const me=await(await fetch(oe)).blob();a.append("hat_image",new File([me],"hat.png",{type:"image/png"}))}catch(j){console.warn("Could not load hat image:",j)}const L=await(await fetch(`${ae}/functions/v1/generate-profile-avatar`,{method:"POST",headers:{Authorization:`Bearer ${v}`},body:a})).json();if(!L.success)throw new Error(L.error||"Generering fejlede");if(!L.image_base64)throw new Error("Avatar mangler i svaret");const N=L.format==="png"?"image/png":"image/webp",C=atob(L.image_base64),ie=new Uint8Array(C.length);for(let j=0;j<C.length;j++)ie[j]=C.charCodeAt(j);const ve=new Blob([ie],{type:N}),be=await Se(ve),K=`${t.institution_id}/${t.id}_${Date.now()}.webp`,{error:Q}=await B.storage.from("profile-pictures").upload(K,be,{contentType:"image/webp",cacheControl:"31536000"});if(Q)throw new Error("Kunne ikke gemme avatar: "+(Q.message||Q));const{data:J,error:re}=await Ae("save_ai_avatar_metadata",()=>B.rpc("save_ai_avatar_metadata",{p_user_id:t.id,p_storage_path:K,p_ai_style:L.ai_style||o.key,p_ai_prompt:L.prompt||""}));if(re||J&&J.success===!1){try{await B.storage.from("profile-pictures").remove([K])}catch{}throw new Error(re?.message||J?.error||"Kunne ikke gemme avatar-metadata")}se(t.id);const ge=await le({id:t.id,profile_picture_url:K,profile_picture_type:"ai_avatar"});s&&URL.revokeObjectURL(s),f.innerHTML=`
                <div class="profile-pic-preview-container">
                ...
                </div>`,f.querySelector("#pp-ai-retry-err")?.addEventListener("click",()=>{f.style.display="none",A.style.display="none",w.style.display="flex"})}}function ye(){w.style.display="none",H&&(H.style.display="none"),M.style.display="block";const i=e.querySelector("#pp-ai-camera-video"),s=e.querySelector("#pp-ai-capture-btn"),u=e.querySelector("#pp-ai-cam-status");navigator.mediaDevices.getUserMedia({video:{facingMode:"user",width:{ideal:800},height:{ideal:800}}}).then(v=>{g(v),i.srcObject=v,s.disabled=!1,u.style.display="none"}).catch(v=>{u.innerHTML=`<span style="color:#f87171;">Kunne ikke starte kamera: ${h(v.message)}</span>`,s.style.display="none"}),s.onclick=()=>{const v=document.createElement("canvas"),a=Math.min(i.videoWidth,i.videoHeight);v.width=800,v.height=800;const S=v.getContext("2d"),_=Math.round(a/1.3),L=(i.videoWidth-_)/2,N=(i.videoHeight-_)/2;S.translate(800,0),S.scale(-1,1),S.drawImage(i,L,N,_,_,0,0,800,800),v.toBlob(C=>{C&&V(C,"Kamera-foto sendes til AI og slettes straks efter","camera")},"image/jpeg",.9)}}R.addEventListener("change",async()=>{const i=R.files?.[0];if(i){R.value="";try{const s=await Z(i);V(s,"Uploadet billede sendes til AI og slettes straks efter","upload")}catch(s){f.style.display="block",f.innerHTML=`<div style="color:#f87171;text-align:center;padding:12px;">${h(s.message)}</div>`}}}),y?V(y,"Referencebillede fra bibliotek","library"):F&&fetch(F).then(i=>i.blob()).then(i=>{i&&(y=i,V(i,"Referencebillede fra bibliotek","library"))}).catch(i=>console.warn("[ai-avatar] Kunne ikke hente reference:",i)),xe(t.id,t).then(async i=>{if(!w)return;const s=new Map,u=i.filter(a=>a.storage_path&&!a.storage_path.startsWith("http")&&a.picture_type!=="library"&&a.picture_type!=="icon");if(u.length>0){const a=u.map(_=>_.storage_path),{data:S}=await B.storage.from("profile-pictures").createSignedUrls(a,3600);S&&S.forEach((_,L)=>{_.signedUrl&&s.set(u[L].id,_.signedUrl)})}const v=i.map((a,S)=>{const _=s.get(a.id)||a.storage_path;return`<div data-lib-index="${S}" style="flex:0 0 auto;width:64px;text-align:center;cursor:pointer;" title="Brug som reference">
            ...
            </div>`}).join("");v?w.innerHTML=v:w.innerHTML='<div style="color:#94a3b8;font-size:11px;padding:8px;">Ingen eksisterende billeder. Brug kamera eller upload ovenfor.</div>',w.querySelectorAll("[data-lib-index]").forEach(a=>{a.addEventListener("click",async()=>{const S=parseInt(a.dataset.libIndex),_=i[S];if(_){a.querySelector("img").style.borderColor="#f59e0b";try{const L=s.get(_.id)||_.storage_path,C=await(await fetch(L)).blob();w.style.display="none",V(C,"Eksisterende billede sendes til AI og slettes straks efter","library")}catch{a.querySelector("img").style.borderColor="#f87171",setTimeout(()=>{a.querySelector("img").style.borderColor="transparent"},1500)}}})})})}export{Ne as openProfilePictureModal};
