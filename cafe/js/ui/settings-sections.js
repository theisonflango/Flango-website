...
(function () {
  ...
  sections['Restaurant Mode'] = {
    render(ctx) {
      ...
      const kitchenOn = window.__flangoToolbarStore.flagByKey(inst, 'kitchen');
      ...
      return `<div class="fsp-page">
        ...
        <div class="fsp-body${enabled ? ' open' : ''}" data-expand-target="rm-body">
          <div class="fsp-section"><div class="fsp-block">
            <div class="fsp-row"><div style="flex:1"><div class="fsp-row-title">Vis køkkenskærm-genvej i toolbar</div><div class="fsp-row-desc">Tip: hold knappen inde for at åbne køkkenskærmen.</div></div><div class="fsp-toggle${kitchenOn ? ' on' : ''}" data-rm-kitchen></div></div>
          </div></div>
    ...
    wire(container, ctx) {
      ...
      wireNumberInputs(container, ctx);
      wireStepButtons(container);
      // Køkkenskærm-genvejen er en TOOLBAR-genvej, ikke en egen kolonne — den går derfor
      // gennem toolbar-lageret og ikke gennem wireToggles' data-field (som betyder "kolonne").
      const _rmKitchenTog = container.querySelector('[data-rm-kitchen]');
      if (_rmKitchenTog) _rmKitchenTog.addEventListener('click', async () => {
        const store = window.__flangoToolbarStore;
        const on = !_rmKitchenTog.classList.contains('on');
        if (!(await ctx.saveFields(store.flagPatch(store.item('kitchen'), on)))) return;
        _rmKitchenTog.classList.toggle('on', on);
  ...
  sections['Toolbar'] = {
    ...
    // i toolbar-items.js), så labels, ikoner og lagring ALTID matcher den rigtige header.
    // Både tilstand og rækkefølge læses/skrives gennem __flangoToolbarStore — samme
    // adgangslag som headeren bruger, så de to ikke kan komme til at læse hver sit sted.
    //   key   = orderKey (persisteret i settings.toolbar.order — rør ikke)
    //   m     = selve mapping-elementet (det tilstand slås op på ved læsning og skrivning)
    ...
    _build(ctx) {
      ...
      // Anvend gemt rækkefølge
      const order = store.order(inst);
      if (order.length) {
        const orderMap = {};
    ...
    _listHtml(items) {
      return items.map(it => {
        ...
        const toggleAttr = it.alwaysOn ? '' : ` data-tb-toggle="${it.key}"`;
    ...
    wire(container, ctx) {
      ...
      function persistOrder() {
        ctx.saveFields(window.__flangoToolbarStore.orderPatch(self._getItems(ctx).map(x => x.key)));
        applyLiveOrder();
      }
      ...
      function setToggle(orderKey, isOn) {
        const inst = ctx.institutionData || {};
        const it = self._getItems(ctx).find(x => x.key === orderKey);
        ...
        ctx.saveFields(window.__flangoToolbarStore.flagPatch(it.m, isOn)); // samme adgangslag som headeren læser
        // shift_timer is gated by a runtime global (clerk-login + pill creation read it).
        if (it.m.key === 'shift_timer' && window.__flangoInstitutionSettings) {
    ...
    wire(container, ctx) {
      ...
      notifyEl.addEventListener('click', async (e) => {
        ...
        if (gem) {
          ...
          // samme. Adressen kræver bekræftelse på den nye postkasse.
          ...
          if (res?.success && res.code_sent && res.change_id) {
            form.dataset.changeId = res.change_id;
            form.innerHTML = `
              <div style="padding:12px 14px;border-radius:10px;background:rgba(244,162,97,0.12);border:1px solid rgba(244,162,97,0.3)">
                ...
                Indtast koden, der er sendt til den nye modtageradresse. Koden gælder i 30 minutter.</div>
                <label style="display:block;margin-top:12px;font-size:12px;color:var(--fsp-txt3)">Kode fra den nye adresse</label>
                ...
                <button class="fsp-btn fsp-btn-ghost" data-action="annuller-mailskift" style="width:100%;margin-top:8px;padding:10px;font-size:13px">Tilbage</button>
                <div data-mail-fejl style="margin-top:8px;font-size:13px;color:#e85a6f"></div>
              </div>`;
            return;
          }
        ...
        const res2 = await api.confirmNotificationEmailChange(form.dataset.changeId, kode);
