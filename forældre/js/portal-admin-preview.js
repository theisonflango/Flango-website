...
(function () {
  ...
  const ALLOWED_HOST_ORIGINS = [
    ...
    'http://localhost:5175',
    'http://127.0.0.1:5175',
    // Skærmtid-dashboardet (Vite). Prod ligger på flango.dk, som allerede står
    // øverst — kun dev-porten mangler.
    'http://localhost:5173',
    'http://127.0.0.1:5173',
    'tauri://localhost',
  ...
  const SECTION_DOM = {
    ...
    game_policy: ['section-game-policy'],
  ...
  const CONTROL_HELP = {
    ...
    game_policy: 'Institutionens pædagogiske grundlag for spilkataloget.',
  ...
  let scope = null;             // sektionsnøgler værten administrerer; null = hele portalen
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      /* Uden for værtens scope: skjult, ikke gråtonet. En vært der kun
         administrerer sit eget område skal ikke vise resten af portalen som
         "slukket" — den er ikke slukket, den hører bare ikke til her. */
      .flango-preview-out-of-scope { display: none !important; }
  ...
  function decorate() {
    ...
    document.querySelectorAll('.section[id^="section-"]').forEach((el) => {
      if (el.classList.contains('flango-preview-out-of-scope')) return;
  ...
  function inScope(key) {
    return !scope || scope.indexOf(key) !== -1;
  }

  function scopedSectionIds() {
    const allowed = new Set();
    (scope || []).forEach((key) => (SECTION_DOM[key] || []).forEach((id) => allowed.add(id)));
    return allowed;
  }

  /**
   * Sidepanelets genveje peger på kortene, så de skal følge samme scope —
   * ellers tilbyder menuen at hoppe til noget der er skjult.
   *
   * Portalen genopbygger #sidebar-nav ved hvert faneskift (også det scope selv
   * udløser), så klasserne kan ikke sættes én gang. Derfor observeres listen i
   * stedet for at gætte hvornår den er færdig.
   */
  function applyScopeToSidebar(allowed) {
    if (!scope) return;
    const ids = allowed || scopedSectionIds();

    document.querySelectorAll('.sidebar-nav-item[data-scroll]').forEach((el) => {
      el.classList.toggle('flango-preview-out-of-scope', !ids.has(el.dataset.scroll));
    });

    // Gruppe-etiketterne er søskende til punkterne, ikke forældre til dem — en
    // etiket skjules derfor når alle punkter FREM TIL næste etiket er væk.
    document.querySelectorAll('#sidebar-nav .sidebar-group-label').forEach((label) => {
      let live = false;
      for (let el = label.nextElementSibling; el; el = el.nextElementSibling) {
        if (el.classList.contains('sidebar-group-label')) break;
        if (el.classList.contains('sidebar-nav-item') && !el.classList.contains('flango-preview-out-of-scope')) {
          live = true;
          break;
        }
      }
      label.classList.toggle('flango-preview-out-of-scope', !live);
    });
  }

  let sidebarScopeObserver = null;
  function watchSidebarScope() {
    if (!scope || sidebarScopeObserver || typeof MutationObserver !== 'function') return;
    const nav = document.getElementById('sidebar-nav');
    if (!nav) return;
    sidebarScopeObserver = new MutationObserver(() => applyScopeToSidebar());
    sidebarScopeObserver.observe(nav, { childList: true });
  }

  /**
   * Skjul alt uden for værtens scope: kort, faner der ender tomme, og
   * bundnavigationen.
   *
   * Portalen renderer stadig HELE portalen — der er ingen scope-gren i
   * render-koden. Scope er udelukkende en preview-dekoration, så en vært aldrig
   * kan komme til at ændre hvad forælderen faktisk ser, og så en fejl her ikke
   * kan skjule noget for en rigtig forælder.
   */
  function applyScope() {
    if (!scope) return;
    ...
    document.querySelectorAll('.section[id^="section-"]').forEach((el) => {
      el.classList.toggle('flango-preview-out-of-scope', !allowed.has(el.id));
    });

    applyScopeToSidebar(allowed);

    // En fane uden synlige kort er en blindgyde — skjul knappen, og sørg for at
    // den fane der FAKTISK har indhold er den aktive.
    //
    // Selektoren rammer BEVIDST kun fane-knapperne. Sidepanelets punkter bærer
    // også data-tab, så et bredt [data-tab="…"] ville tænde dem igen lige efter
    // applyScopeToSidebar havde slukket dem — netop det gjorde det umuligt at
    // skjule Grænser-fanens øvrige punkter.
    const TAB_BTN = '.dtab-item, .bnav-item';
    let firstLiveTab = null;
    document.querySelectorAll('.tab-view').forEach((view) => {
      const live = view.querySelector('.section[id^="section-"]:not(.flango-preview-out-of-scope)');
      if (live && !firstLiveTab) firstLiveTab = view.id;
      document.querySelectorAll(TAB_BTN).forEach((btn) => {
        if (btn.dataset.tab !== view.id) return;
        btn.classList.toggle('flango-preview-out-of-scope', !live);
      });
    });

    if (firstLiveTab) {
      const active = document.querySelector('.tab-view.active');
      if (!active || active.classList.contains('flango-preview-out-of-scope') ||
          !active.querySelector('.section[id^="section-"]:not(.flango-preview-out-of-scope)')) {
        // Efter renderApp, ikke midt i den: decorate() kaldes fra render-stien,
        // og et faneskift derinde kører portalens egne handlers oven i en
        // halvfærdig opsætning. Udskudt til efter kaldstakken er tømt.
        setTimeout(() => {
          const btn = [...document.querySelectorAll(TAB_BTN)].find((b) => b.dataset.tab === firstLiveTab);
          if (btn) btn.click();
        }, 0);
      }
    }
  ...
  function bootstrap(opts) {
    ...
    return new Promise((resolve) => {
      ...
      async function onSessionMessage(event) {
        ...
        // Værts-scope: en liste af sektionsnøgler værten administrerer. Tom
        // eller fraværende = hele portalen (caféen og super-admin). Skærmtid-
        // dashboardet sender sine egne sektioner, så personalet dér kun ser og
        // kan røre det der hører til skærmtid.
        scope = (Array.isArray(msg.scope) && msg.scope.length) ? msg.scope.slice() : null;
        try {
